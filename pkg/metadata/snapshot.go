...
func (s *snapshotter) resolveKey(ctx context.Context, key string) (string, error) {
	...
	if err := view(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if id == "" {
			return fmt.Errorf("snapshot %v does not exist: %w", key, over_errdefs.ErrNotFound)
		}
		return nil
	}); err != nil {
		return "", err
...
func (s *snapshotter) Update(ctx context.Context, info snapshots.Info, fieldpaths ...string) (snapshots.Info, error) {
	...
	if info.Name == "" {
		return snapshots.Info{}, over_errdefs.ErrInvalidArgument
	}
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		bkt := getSnapshotterBucket(tx, ns, s.name)
		if bkt == nil {
			return fmt.Errorf("snapshot %v does not exist: %w", info.Name, over_errdefs.ErrNotFound)
		}
...
func (s *snapshotter) createSnapshot(ctx context.Context, key, parent string, readonly bool, opts []snapshots.Opt) ([]mount.Mount, error) {
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if target != "" {
			if tbkt := bkt.Bucket([]byte(target)); tbkt != nil {
				rerr = fmt.Errorf("target snapshot %q: %w", target, over_errdefs.ErrAlreadyExists)
		...
		if bbkt := bkt.Bucket([]byte(key)); bbkt != nil {
			rerr = fmt.Errorf("snapshot %q: %w", key, over_errdefs.ErrAlreadyExists)
		...
		if parent != "" {
			...
			if pbkt == nil {
				return fmt.Errorf("parent snapshot %v does not exist: %w", parent, over_errdefs.ErrNotFound)
			}
	...
	if over_errdefs.IsAlreadyExists(err) {
		if target != "" {
			...
			if tinfo == nil {
				return nil, fmt.Errorf("target snapshot %q in backend: %w", target, over_errdefs.ErrNotFound)
			}
			...
			rerr = fmt.Errorf("target snapshot %q from snapshotter: %w", target, over_errdefs.ErrAlreadyExists)
		} else {
			...
			return nil, fmt.Errorf("unexpected error from snapshotter: %v: %w", err, over_errdefs.ErrUnknown)
		}
	} else if err != nil {
		return nil, err
	} else {
	...
	if txerr := update(ctx, s.db, func(tx *bolt.Tx) error {
		bkt := getSnapshotterBucket(tx, ns, s.name)
		if bkt == nil {
			return fmt.Errorf("can not find snapshotter %q: %w", s.name, over_errdefs.ErrNotFound)
		}
		...
		if parent != "" {
			...
			if bparent != string(pbkt.Get(bucketKeyName)) {
				return fmt.Errorf("mismatched parent %s from target %s: %w", parent, target, over_errdefs.ErrInvalidArgument)
			}
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if bkt == nil {
			return fmt.Errorf("can not find snapshotter %q: %w",
				s.name, over_errdefs.ErrNotFound)
		}
		...
		if err != nil {
			if err == bolt.ErrBucketExists {
				rerr = fmt.Errorf("snapshot %q: %w", name, over_errdefs.ErrAlreadyExists)
				return nil
			}
			return err
		}
		...
		if len(parent) > 0 {
			...
			if pbkt == nil {
				return fmt.Errorf("parent snapshot %v does not exist: %w", string(parent), over_errdefs.ErrNotFound)
			}
