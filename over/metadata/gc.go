...
func startGCContext(ctx context.Context, collectors map[gc.ResourceType]Collector) *gcContext {
	...
	labelHandlers := []referenceLabelHandler{
		{
			key: labelGCContentRef, // containerd.io/gc.ref.content
		...
		{
			key: labelGCSnapRef, // containerd.io/gc.ref.snapshot.
			fn: func(ns string, k, v []byte, fn func(gc.Node)) {
	...
	for k, v := v1c.First(); k != nil; k, v = v1c.Next() {
		...
		if lbkt != nil {
			if err := lbkt.ForEach(func(k, v []byte) error {
				...
				if lblbkt := libkt.Bucket(bucketKeyObjectLabels); lblbkt != nil { // labels
					if expV := lblbkt.Get(labelGCExpire); expV != nil { // containerd.io/gc.expire
					...
					if flatV := lblbkt.Get(labelGCFlat); flatV != nil { // containerd.io/gc.flat
						flat = true
					}
				}
