...
func NewLeaseManager(db *DB) leases2.Manager {
	return &leaseManager{
		db: db,
	}
}
...
func (lm *leaseManager) Create(ctx context.Context, opts ...leases2.Opt) (leases2.Lease, error) {
	var l leases2.Lease
	for _, opt := range opts {
		if err := opt(&l); err != nil {
			return leases2.Lease{}, err
		}
	}
	if l.ID == "" {
		return leases2.Lease{}, errors.New("lease id must be provided")
	}
...
func (lm *leaseManager) Delete(ctx context.Context, lease leases2.Lease, _ ...leases2.DeleteOpt) error {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return err
	}
...
func (lm *leaseManager) List(ctx context.Context, fs ...string) ([]leases2.Lease, error) {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return nil, err
	}
...
func (lm *leaseManager) AddResource(ctx context.Context, lease leases2.Lease, r leases2.Resource) error {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return err
	}
...
func (lm *leaseManager) DeleteResource(ctx context.Context, lease leases2.Lease, r leases2.Resource) error {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return err
	}
...
func (lm *leaseManager) ListResources(ctx context.Context, lease leases2.Lease) ([]leases2.Resource, error) {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return nil, err
	}

	var rs []leases2.Resource
...
func addSnapshotLease(ctx context.Context, tx *bolt.Tx, snapshotter, key string) error {
	lid, ok := leases2.FromContext(ctx)
	if !ok {
		return nil
	}
...
func parseLeaseResource(r leases2.Resource) ([]string, string, error) {
	var (
		ref  = r.ID
		typ  = r.Type
