...
func (lm *leaseManager) Delete(ctx context.Context, lease leases.Lease, _ ...leases.DeleteOpt) error {
	...
	return update(ctx, lm.db, func(tx *bolt.Tx) error {
		...
		if topbkt == nil {
			return fmt.Errorf("lease %q: %w", lease.ID, over_errdefs.ErrNotFound)
		}
		if err := topbkt.DeleteBucket([]byte(lease.ID)); err != nil {
			if err == bolt.ErrBucketNotFound {
				err = fmt.Errorf("lease %q: %w", lease.ID, over_errdefs.ErrNotFound)
			}
			return err
		}
...
func addSnapshotLease(ctx context.Context, tx *bolt.Tx, snapshotter, key string) error {
	...
	if bkt == nil {
		return fmt.Errorf("lease does not exist: %w", over_errdefs.ErrNotFound)
	}
...
func addIngestLease(ctx context.Context, tx *bolt.Tx, ref string) (bool, error) {
	...
	bkt := getBucket(tx, bucketKeyVersion, []byte(namespace), bucketKeyObjectLeases, []byte(lid))
	if bkt == nil {
		return false, fmt.Errorf("lease does not exist: %w", over_errdefs.ErrNotFound)
	}
...
func parseLeaseResource(r leases.Resource) ([]string, string, error) {
	...
	case string(bucketKeyObjectContent),
		...
		if len(keys) != 1 {
			return nil, "", fmt.Errorf("invalid resource type %s: %w", typ, over_errdefs.ErrInvalidArgument)
		}

		if k == string(bucketKeyObjectContent) {
			dgst, err := digest.Parse(ref)
			if err != nil {
				return nil, "", fmt.Errorf("invalid content resource id %s: %v: %w", ref, err, over_errdefs.ErrInvalidArgument)
			}
			ref = dgst.String()
		}
	case string(bucketKeyObjectSnapshots):
		if len(keys) != 2 {
			return nil, "", fmt.Errorf("invalid snapshot resource type %s: %w", typ, over_errdefs.ErrInvalidArgument)
		}
	default:
		return nil, "", fmt.Errorf("resource type %s not supported yet: %w", typ, over_errdefs.ErrNotImplemented)
	}
