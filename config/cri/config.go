...
type CniConfig struct {
	...
	NetworkPluginSetupSerially bool `toml:"setup_serially" json:"setupSerially"` // 如果存在多个CNI插件配置文件并且NetworkPluginMaxConfNum大于1，则containerd是否连续设置网络。
...
type PluginConfig struct {
	...
	// NetNSMountsUnderStateDir 将所有网络名称空间的挂载放在 ${StateDir}/netns下，而不是放在硬编码目录/var/run/netns下更改此设置需要删除所有容器。
