...
func validateTopic(topic string) error {
	if topic == "" {
		return fmt.Errorf("must not be empty: %w", over_errdefs.ErrInvalidArgument)
	}

	if topic[0] != '/' {
		return fmt.Errorf("must start with '/': %w", over_errdefs.ErrInvalidArgument)
	}

	if len(topic) == 1 {
		return fmt.Errorf("must have at least one component: %w", over_errdefs.ErrInvalidArgument)
	}
...
func validateEnvelope(envelope *events.Envelope) error {
	...
	if envelope.Timestamp.IsZero() {
		return fmt.Errorf("timestamp must be set on forwarded event: %w", over_errdefs.ErrInvalidArgument)
	}
