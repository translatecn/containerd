...
import (
	...
	"demo/over/cri/sbserver/podsandbox"
	"demo/over/errdefs"
...
func (c *CriService) recover(ctx context.Context) error {
	...
	for _, cleanup := range []struct {
		...
		{
			cntrs:  sandboxes,
			base:   filepath.Join(c.config.RootDir, sandboxesDir), // /var/lib/containerd/io.containerd.grpc.v1.cri
			errMsg: "failed to cleanup orphaned sandbox directories",
		},
		{
			cntrs:  sandboxes,
			base:   filepath.Join(c.config.StateDir, sandboxesDir), // /run/containerd/io.containerd.grpc.v1.cri
			errMsg: "failed to cleanup orphaned volatile sandbox directories",
		},
		{
			cntrs:  containers,
			base:   filepath.Join(c.config.RootDir, containersDir), // /var/lib/containerd/io.containerd.grpc.v1.cri
			errMsg: "failed to cleanup orphaned container directories",
		},
		{
			cntrs:  containers,
			base:   filepath.Join(c.config.StateDir, containersDir), // /run/containerd/io.containerd.grpc.v1.cri
