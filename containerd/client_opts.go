...
type clientOpts struct {
	...
	defaultPlatform over_platforms.MatchComparer
	services        *services
...
func WithDefaultPlatform(platform over_platforms.MatchComparer) ClientOpt {
	return func(c *clientOpts) error {
...
func WithPlatform(platform string) RemoteOpt {
	if platform == "" {
		platform = over_platforms.DefaultString()
	}
...
func WithPlatformMatcher(m over_platforms.MatchComparer) RemoteOpt {
	return func(_ *Client, c *RemoteContext) error {
		c.PlatformMatcher = m
		return nil
	}
...
// for content.
// The default is `over_images.ChildGCLabels`.
...
func WithImageHandler(h over_images.Handler) RemoteOpt {
	return func(client *Client, c *RemoteContext) error {
...
func WithImageHandlerWrapper(w func(over_images.Handler) over_images.Handler) RemoteOpt {
	return func(client *Client, c *RemoteContext) error {
		c.HandlerWrapper = w
		return nil
	}
