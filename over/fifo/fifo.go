...
func OpenFifo(ctx context.Context, filePath string, flag int, perm os.FileMode) (io.ReadWriteCloser, error) {
	fifo, err := openFifo(ctx, filePath, flag, perm)
	if fifo == nil {
...
func openFifo(ctx context.Context, filePath string, flag int, perm os.FileMode) (*fifo, error) {
	if _, err := os.Stat(filePath); err != nil {
		if os.IsNotExist(err) && flag&syscall.O_CREAT != 0 {
			if err := syscall.Mkfifo(filePath, uint32(perm&os.ModePerm)); err != nil && !os.IsExist(err) {
				return nil, fmt.Errorf("error creating fifo %v: %w", filePath, err)
			}
		} else {
			return nil, err
		}
