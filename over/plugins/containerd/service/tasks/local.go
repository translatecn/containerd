...
import (
	...
	api "demo/over/api/services/tasks/v1"
	"demo/over/api/types"
...
func initFunc(ic *plugin.InitContext) (interface{}, error) {
	...
	l := &local{
		runtimes:   runtimes,
		containers: metadata2.NewContainerStore(db),
...
func (l *local) Create(ctx context.Context, r *api.CreateTaskRequest, _ ...grpc.CallOption) (*api.CreateTaskResponse, error) {
	...
	// 向 containerd-shim-runc-v2 发送请求,创建一个 task
	c, err := rtime.Create(ctx, r.ContainerID, opts) // 通过 containerd-shim-runc-v2 启动子进程，会暴露一个unix 套接字
	if err != nil {
		return nil, errdefs.ToGRPC(err)
	}
	labels := map[string]string{"runtime": container.Runtime.Name} // io.containerd.runc.v2
	if err := l.monitor.Monitor(c, labels); err != nil {
		return nil, fmt.Errorf("monitor task: %w", err)
	}
	pid, err := c.PID(ctx) // 26277
	// containerd-shim(26256)─┬─runc:[2:INIT](26277)─┬─{runc:[2:INIT]}(26278)
	//                       │                      ├─{runc:[2:INIT]}(26279)
	//                       │                      ├─{runc:[2:INIT]}(26280)
	//                       │                      ├─{runc:[2:INIT]}(26281)
	//                       │                      └─{runc:[2:INIT]}(26282)
	//                       ├─{containerd-shim}(26257)// 线程
	//                       ├─{containerd-shim}(26258)
	//                       ├─{containerd-shim}(26259)
	//                       ├─{containerd-shim}(26260)
	//                       ├─{containerd-shim}(26261)
	//                       ├─{containerd-shim}(26262)
	//                       ├─{containerd-shim}(26263)
	//                       ├─{containerd-shim}(26264)
	//                       ├─{containerd-shim}(26265)
	//                       └─{containerd-shim}(26266)
	if err != nil {
		return nil, fmt.Errorf("failed to get task pid: %w", err)
	}
	return &api.CreateTaskResponse{
		ContainerID: r.ContainerID, // nginx_1
		Pid:         pid,
	}, nil
}
...
func (l *local) Start(ctx context.Context, r *api.StartRequest, _ ...grpc.CallOption) (*api.StartResponse, error) {
	...
	// root       26927       1  0 10:31 pts/2    00:00:00 /Users/acejilam/Desktop/containerd/containerd-shim-runc-v2 -namespace default -id nginx_1 -address /run/containerd/containerd.sock
	// root       26963   26927  0 10:31 pts/0    00:00:00 sh
...
func getCheckpointPath(runtime string, option *ptypes.Any) (string, error) {
	...
	case checkRuntime(runtime, "io.containerd.runc"):
		...
		opts, ok := v.(*runc.CheckpointOptions)
		if !ok {
