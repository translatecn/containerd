...
func Validate(s string) error {
	if len(s) == 0 {
		return fmt.Errorf("identifier must not be empty: %w", over_errdefs.ErrInvalidArgument)
	}

	if len(s) > maxLength {
		return fmt.Errorf("identifier %q greater than maximum length (%d characters): %w", s, maxLength, over_errdefs.ErrInvalidArgument)
	}

	//if !identifierRe.MatchString(s) {
	//	return fmt.Errorf("identifier %q must match %v: %w", s, identifierRe, over_errdefs.ErrInvalidArgument)
	//}
	return nil
}
