...
import (
	...
	"demo/others/imgcrypt/cmd/ctr/commands/flags"
...
`,
	...
	Action: func(context *cli.Context) error {
		...
		if len(p) == 0 {
			p = append(p, over_platforms.DefaultSpec())
		}
		...
		for _, platform := range p {
			fmt.Printf("unpacking %s %s...\n", over_platforms.Format(platform), img.Target.Digest)
			i := containerd.NewImageWithPlatform(client, img, over_platforms.Only(platform))
