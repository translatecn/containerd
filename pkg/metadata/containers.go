...
func (s *containerStore) Get(ctx context.Context, id string) (containers.Container, error) {
	...
	if err := view(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if bkt == nil {
			return fmt.Errorf("container %q in namespace %q: %w", id, namespace, over_errdefs.ErrNotFound)
		}
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if err != nil {
			if err == bolt.ErrBucketExists {
				err = fmt.Errorf("container %q: %w", container.ID, over_errdefs.ErrAlreadyExists)
			}
			return err
		}
...
func (s *containerStore) Update(ctx context.Context, container containers.Container, fieldpaths ...string) (containers.Container, error) {
	...
	if container.ID == "" {
		return containers.Container{}, fmt.Errorf("must specify a container id: %w", over_errdefs.ErrInvalidArgument)
	}
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		bkt := getContainersBucket(tx, namespace)
		if bkt == nil {
			return fmt.Errorf("cannot update container %q in namespace %q: %w", container.ID, namespace, over_errdefs.ErrNotFound)
		}

		cbkt := bkt.Bucket([]byte(container.ID))
		if cbkt == nil {
			return fmt.Errorf("container %q: %w", container.ID, over_errdefs.ErrNotFound)
		}
		...
		if len(fieldpaths) == 0 {
			...
			if updated.Snapshotter != container.Snapshotter {
				return fmt.Errorf("container.Snapshotter field is immutable: %w", over_errdefs.ErrInvalidArgument)
			}

			if updated.Runtime.Name != container.Runtime.Name {
				return fmt.Errorf("container.Runtime.Name field is immutable: %w", over_errdefs.ErrInvalidArgument)
			}
		}
		...
		for _, path := range fieldpaths {
			...
			default:
				return fmt.Errorf("cannot update %q field on %q: %w", path, container.ID, over_errdefs.ErrInvalidArgument)
			}
		}
...
func (s *containerStore) Delete(ctx context.Context, id string) error {
	...
	return update(ctx, s.db, func(tx *bolt.Tx) error {
		bkt := getContainersBucket(tx, namespace)
		if bkt == nil {
			return fmt.Errorf("cannot delete container %q in namespace %q: %w", id, namespace, over_errdefs.ErrNotFound)
		}

		if err := bkt.DeleteBucket([]byte(id)); err != nil {
			if err == bolt.ErrBucketNotFound {
				err = fmt.Errorf("container %v: %w", id, over_errdefs.ErrNotFound)
			}
			return err
		}
...
func validateContainer(container *containers.Container) error {
	...
	for k := range container.Extensions {
		if k == "" {
			return fmt.Errorf("container.Extension keys must not be zero-length: %w", over_errdefs.ErrInvalidArgument)
		}
	}
	...
	if container.Runtime.Name == "" {
		return fmt.Errorf("container.Runtime.Name must be set: %w", over_errdefs.ErrInvalidArgument)
	}

	if container.Spec == nil {
		return fmt.Errorf("container.Spec must be set: %w", over_errdefs.ErrInvalidArgument)
	}

	if container.SnapshotKey != "" && container.Snapshotter == "" {
		return fmt.Errorf("container.Snapshotter must be set if container.SnapshotKey is set: %w", over_errdefs.ErrInvalidArgument)
	}
