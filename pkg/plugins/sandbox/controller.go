...
import (
	...
	"demo/over/errdefs"
	"demo/over/platforms"
	runtimeAPI "demo/pkg/api/runtime/sandbox/v1"
	"demo/pkg/events"
...
func init() {
	over_plugin2.Register(&over_plugin2.Registration{
		Type: over_plugin2.SandboxControllerPlugin,
		ID:   "local",
		...
		InitFn: func(ic *over_plugin2.InitContext) (interface{}, error) {
			shimPlugin, err := ic.GetByID(over_plugin2.RuntimePluginV2, "shim")
			if err != nil {
				return nil, err
			}

			exchangePlugin, err := ic.GetByID(over_plugin2.EventPlugin, "exchange")
			if err != nil {
				return nil, err
			}

			sbPlugin, err := ic.GetByID(over_plugin2.SandboxStorePlugin, "local")
			if err != nil {
				return nil, err
			}
...
func (c *controllerLocal) Create(ctx context.Context, sandboxID string, opts ...sandbox2.CreateOpt) error {
	...
	if _, err := c.shims.Get(ctx, sandboxID); err == nil {
		return fmt.Errorf("sandbox %s already running: %w", sandboxID, over_errdefs.ErrAlreadyExists)
	}
	...
	if _, err := svc.CreateSandbox(ctx, &runtimeAPI.CreateSandboxRequest{
		...
		return fmt.Errorf("failed to create sandbox %s: %w", sandboxID, over_errdefs.FromGRPC(err))
	}
...
func (c *controllerLocal) Start(ctx context.Context, sandboxID string) (sandbox2.ControllerInstance, error) {
	...
	if err != nil {
		c.cleanupShim(ctx, sandboxID, svc)
		return sandbox2.ControllerInstance{}, fmt.Errorf("failed to start sandbox %s: %w", sandboxID, over_errdefs.FromGRPC(err))
	}
...
func (c *controllerLocal) Platform(ctx context.Context, sandboxID string) (over_platforms.Platform, error) {
	svc, err := c.getSandbox(ctx, sandboxID)
	if err != nil {
		return over_platforms.Platform{}, err
	}

	response, err := svc.Platform(ctx, &runtimeAPI.PlatformRequest{SandboxID: sandboxID})
	if err != nil {
		return over_platforms.Platform{}, fmt.Errorf("failed to get sandbox platform: %w", over_errdefs.FromGRPC(err))
	}

	var platform over_platforms.Platform
...
func (c *controllerLocal) Stop(ctx context.Context, sandboxID string, opts ...sandbox2.StopOpt) error {
	...
	if _, err := svc.StopSandbox(ctx, req); err != nil {
		return fmt.Errorf("failed to stop sandbox: %w", over_errdefs.FromGRPC(err))
	}
...
func (c *controllerLocal) Shutdown(ctx context.Context, sandboxID string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to shutdown sandbox: %w", over_errdefs.FromGRPC(err))
	}
...
func (c *controllerLocal) Wait(ctx context.Context, sandboxID string) (sandbox2.ExitStatus, error) {
	...
	if err != nil {
		return sandbox2.ExitStatus{}, fmt.Errorf("failed to wait sandbox %s: %w", sandboxID, over_errdefs.FromGRPC(err))
	}
