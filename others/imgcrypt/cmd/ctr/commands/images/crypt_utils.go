...
import (
	...
	"demo/containerd"
	"demo/others/imgcrypt/cmd/ctr/commands/img"
	imgenc "demo/others/imgcrypt/images/encryption"
	"demo/others/imgcrypt/images/encryption/parsehelpers"
	"demo/over/images"
	"demo/over/platforms"
...
func isUserSelectedPlatform(platform *ocispec.Platform, platformList []ocispec.Platform) bool {
	...
	matcher := over_platforms.NewMatcher(*platform)
...
func cryptImage(client *containerd.Client, ctx gocontext.Context, name, newName string, cc *encconfig.CryptoConfig, layers []int32, platformList []string, encrypt bool) (over_images.Image, error) {
...
func encryptImage(client *containerd.Client, ctx gocontext.Context, name, newName string, cc *encconfig.CryptoConfig, layers []int32, platformList []string) (over_images.Image, error) {
	return cryptImage(client, ctx, name, newName, cc, layers, platformList, true)
}

func decryptImage(client *containerd.Client, ctx gocontext.Context, name, newName string, cc *encconfig.CryptoConfig, layers []int32, platformList []string) (over_images.Image, error) {
...
func parsePlatformArray(specifiers []string) ([]ocispec.Platform, error) {
	...
	for _, specifier := range specifiers {
		spec, err := over_platforms.Parse(specifier)
		if err != nil {
