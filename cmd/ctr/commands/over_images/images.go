...
var checkCommand = cli.Command{
	...
	Action: func(context *cli.Context) error {
		...
		for _, image := range imageList {
			...
			available, required, present, missing, err := over_images.Check(ctx, contentStore, image.Target(), over_platforms.Default())
			if err != nil {
				if exitErr == nil {
	...
	Action: func(context *cli.Context) error {
		...
		for i, target := range context.Args() {
			...
			if context.Bool("sync") && i == context.NArg()-1 {
				opts = append(opts, over_images.SynchronousDelete())
			}
...
var pruneCommand = cli.Command{
	...
	Action: func(context *cli.Context) error {
		...
		var delOpts []over_images.DeleteOpt
		for i, imageName := range removedImages {
			...
			if i == len(removedImages)-1 {
				delOpts = []over_images.DeleteOpt{over_images.SynchronousDelete()}
			}
