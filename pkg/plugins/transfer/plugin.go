...
func init() {
	over_plugin2.Register(&over_plugin2.Registration{
		Type: over_plugin2.TransferPlugin,
		ID:   "local",
		...
		InitFn: func(ic *over_plugin2.InitContext) (interface{}, error) {
			...
			ms := m.(*metadata.DB)
			l, err := ic.Get(over_plugin2.LeasePlugin)
			if err != nil {
				return nil, err
			}
			...
			for _, uc := range config.UnpackConfiguration {
				p, err := over_platforms.Parse(uc.Platform)
				if err != nil {
					return nil, fmt.Errorf("%s: platform configuration %v invalid", over_plugin2.TransferPlugin, uc.Platform)
				}

				sn := ms.Snapshotter(uc.Snapshotter)
				if sn == nil {
					return nil, fmt.Errorf("snapshotter %q not found: %w", uc.Snapshotter, over_errdefs.ErrNotFound)
				}

				diffPlugins, err := ic.GetByType(over_plugin2.DiffPlugin)
				if err != nil {
					return nil, fmt.Errorf("error loading diff plugins: %w", err)
				}
				var applier diff.Applier
				target := over_platforms.OnlyStrict(p)
				if uc.Differ != "" {
					plugin, ok := diffPlugins[uc.Differ]
					if !ok {
						return nil, fmt.Errorf("diff plugin %q: %w", uc.Differ, over_errdefs.ErrNotFound)
					}
				...
				if applier == nil {
					return nil, fmt.Errorf("no matching diff plugins: %w", over_errdefs.ErrNotFound)
				}
...
func defaultConfig() *transferConfig {
	return &transferConfig{
		...
		UnpackConfiguration: []unpackConfiguration{
			{
				Platform:    over_platforms.Format(over_platforms.DefaultSpec()),
