...
import (
	"crypto/tls"
	portforward2 "demo/over/cri/streaming/portforward"
	remotecommand2 "demo/over/cri/streaming/remotecommand"
	"errors"
	"io"
	"net"
	"net/http"
...
type Config struct {
	...
	// demo/third_party/over/k8s.io/kubernetes/pkg/kubelet/server/remotecommand/constants.go for available protocols.
	// Only used for SPDY streaming.
	SupportedRemoteCommandProtocols []string

	// The streaming protocols the server supports (understands and permits).  See
	// demo/third_party/over/k8s.io/kubernetes/pkg/kubelet/server/portforward/constants.go for available protocols.
	// Only used for SPDY streaming.
...
var DefaultConfig = Config{
	...
	SupportedPortForwardProtocols:   portforward2.SupportedProtocols,
}
...
func (s *server) serveAttach(req *restful.Request, resp *restful.Response) {
	...
	streamOpts := &remotecommand2.Options{
		Stdin:  attach.Stdin,
		Stdout: attach.Stdout,
		Stderr: attach.Stderr,
		TTY:    attach.Tty,
	}
	remotecommand2.ServeAttach(
		resp.ResponseWriter,
		req.Request,
		s.runtime,
		"", // unused: podName
...
func (s *server) servePortForward(req *restful.Request, resp *restful.Response) {
	...
	portForwardOptions, err := portforward2.BuildV4Options(pf.Port)
	if err != nil {
		resp.WriteError(http.StatusBadRequest, err)
		return
	}

	portforward2.ServePortForward(
		resp.ResponseWriter,
		req.Request,
		s.runtime,
		pf.PodSandboxId,
...
var _ remotecommand2.Executor = &criAdapter{}
var _ remotecommand2.Attacher = &criAdapter{}
var _ portforward2.PortForwarder = &criAdapter{}
