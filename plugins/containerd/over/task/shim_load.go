...
func (m *ShimManager) loadShims(ctx context.Context) error {
	...
	for _, sd := range shimDirs {
		...
		bundle, err := v2.LoadBundle(ctx, m.state, id)
		if err != nil {
		...
		binaryCall := v2.ShimBinary(bundle,
			v2.ShimBinaryConfig{
				Runtime:      runtime,
		...
		instance, err := v2.LoadShim(ctx, bundle, func() {
			log.G(ctx).WithField("id", id).Info("shim disconnected")

			v2.CleanupAfterDeadShim(cleanup.Background(ctx), id, m.shims, m.events, binaryCall)
			// Remove self from the runtime task list.
			m.shims.Delete(ctx, id)
		})
		if err != nil {
			v2.CleanupAfterDeadShim(ctx, id, m.shims, m.events, binaryCall)
			continue
		}
		shim, err := v2.NewShimTask(instance)
		if err != nil {
			return err
		}
