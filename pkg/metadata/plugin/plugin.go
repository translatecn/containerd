...
func (bc *BoltConfig) Validate() error {
	...
	case SharingPolicyShared, SharingPolicyIsolated:
		return nil
	default:
		return fmt.Errorf("unknown policy: %s: %w", bc.ContentSharingPolicy, over_errdefs.ErrInvalidArgument)
	}
}

func init() {
	over_plugin2.Register(&over_plugin2.Registration{
		Type: over_plugin2.MetadataPlugin,
		ID:   "bolt",
		Requires: []over_plugin2.Type{
			over_plugin2.ContentPlugin,
		...
		InitFn: func(ic *over_plugin2.InitContext) (interface{}, error) {
			...
			cs, err := ic.Get(over_plugin2.ContentPlugin)
			if err != nil {
				return nil, err
			}

			snapshottersRaw, err := ic.GetByType(over_plugin2.SnapshotPlugin)
			if err != nil {
				return nil, err
			}
