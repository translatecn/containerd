...
type NetNS interface {
	...
	// 在当前对象的网络命名空间中执行传递的闭包，尝试在返回之前恢复原始命名空间。
	//然而，由于每个操作系统线程可以有不同的网络命名空间，并且Go的线程调度非常不稳定，
	//因此调用者不能保证设置任何特定的命名空间，除非需要该命名空间的操作被Do()包装。
	//此外，从Do()调用的任何代码都不应该调用runtime.UnlockOSThread()，否则在错误命名空间中执行代码的风险将更大。
	//有关详细信息，请参阅https://github.com/golang/go/wiki/LockOSThread。
