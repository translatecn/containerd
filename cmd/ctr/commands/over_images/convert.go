	...
	Action: func(context *cli.Context) error {
		...
		if !context.Bool("all-platforms") {
			if pss := context.StringSlice("platform"); len(pss) > 0 {
				...
				for _, ps := range pss {
					p, err := over_platforms.Parse(ps)
					if err != nil {
						return fmt.Errorf("invalid platform %q: %w", ps, err)
					}
					all = append(all, p)
				}
				convertOpts = append(convertOpts, converter.WithPlatform(over_platforms.Ordered(all...)))
			} else {
				convertOpts = append(convertOpts, converter.WithPlatform(over_platforms.DefaultStrict()))
			}
		}
