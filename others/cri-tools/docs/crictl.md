...
`crictl` is currently in Beta and still under quick iterations. It is hosted at the [cri-tools](https://demo/others/cri-tools) repository. We encourage the CRI developers to report bugs or help extend the coverage by adding more functionalities.

## Install crictl

> **NOTE:** The below steps are based on linux-amd64, however you can get downloads for all other platforms (Windows, ARM, etc) in the [releases page](https://demo/others/cri-tools/release).

`crictl` can be downloaded from cri-tools [release page](https://demo/others/cri-tools/releases):

- using `wget`:

```sh
VERSION="v1.26.0" # check latest version in /releases page
wget https://demo/others/cri-tools/releases/download/$VERSION/crictl-$VERSION-linux-amd64.tar.gz
...
```sh
VERSION="v1.26.0" # check latest version in /releases page
curl -L https://demo/others/cri-tools/releases/download/$VERSION/crictl-${VERSION}-linux-amd64.tar.gz --output crictl-${VERSION}-linux-amd64.tar.gz
sudo tar zxvf crictl-$VERSION-linux-amd64.tar.gz -C /usr/local/bin
rm -f crictl-$VERSION-linux-amd64.tar.gz
```
...
* Visit [kubernetes-sigs/cri-tools](https://demo/others/cri-tools) for more information.
