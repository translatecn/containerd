...
import (
	...
	"demo/others/nri_extend/pkg/log"
	"demo/others/nri_extend/pkg/net"
	"demo/others/nri_extend/pkg/net/multiplex"
	"demo/over/ttrpc"
)
...
type plugin struct {
	...
	rpcs   *ttrpc.Server
	events EventMask // plugin 注册会处理的事件
	closed bool
...
func (p *plugin) StateChange(ctx context.Context, evt *StateChangeEvent) error {
	...
	if err != nil {
		if isFatalError(err) {
			log.Errorf(ctx, "closing plugin %s, failed to handle event %d: %v", p.name(), evt.Event, err)
			p.close()
			return nil
		}
		return err
