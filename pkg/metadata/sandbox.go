...
func (s *sandboxStore) Update(ctx context.Context, sandbox api.Sandbox, fieldpaths ...string) (api.Sandbox, error) {
	...
	if err := update(ctx, s.db, func(tx *bbolt.Tx) error {
		...
		if parent == nil {
			return fmt.Errorf("no sandbox buckets: %w", over_errdefs.ErrNotFound)
		}
		...
		if len(fieldpaths) == 0 {
			...
			if updated.Runtime.Name != sandbox.Runtime.Name {
				return fmt.Errorf("sandbox.Runtime.Name field is immutable: %w", over_errdefs.ErrInvalidArgument)
			}
		}
		...
		for _, path := range fieldpaths {
			...
			default:
				return fmt.Errorf("cannot update %q field on sandbox %q: %w", path, sandbox.ID, over_errdefs.ErrInvalidArgument)
			}
		}
...
func (s *sandboxStore) write(parent *bbolt.Bucket, instance *api.Sandbox, overwrite bool) error {
	...
	if overwrite {
		...
		if bucket != nil {
			return fmt.Errorf("sandbox bucket %q already exists: %w", instance.ID, over_errdefs.ErrAlreadyExists)
		}
...
func (s *sandboxStore) read(parent *bbolt.Bucket, id []byte) (api.Sandbox, error) {
	...
	bucket := parent.Bucket(id)
	if bucket == nil {
		return api.Sandbox{}, fmt.Errorf("bucket %q not found: %w", id, over_errdefs.ErrNotFound)
	}
...
func (s *sandboxStore) validate(new *api.Sandbox) error {
	...
	if new.CreatedAt.IsZero() {
		return fmt.Errorf("creation date must not be zero: %w", over_errdefs.ErrInvalidArgument)
	}

	if new.UpdatedAt.IsZero() {
		return fmt.Errorf("updated date must not be zero: %w", over_errdefs.ErrInvalidArgument)
	}
