...
import (
	...
	csapi "demo/over/api/services/content/v1"
	diffapi "demo/over/api/services/diff/v1"
	ssapi "demo/over/api/services/snapshots/v1"
	csproxy "demo/over/content/proxy"
	"demo/over/diff"
	diffproxy "demo/over/diff/proxy"
...
func New(ctx context.Context, config *srvconfig.Config) (*Server, error) {
	...
	serverOpts := []grpc.ServerOption{
		...
		grpc.UnaryInterceptor(grpc_middleware.ChainUnaryServer(
			...
			unaryNamespaceInterceptor, logGRPC,
		)),
	}
	...
	var (
		...
		ttrpcServices []api.TtrpcService
	...
	for _, p := range plugins {
		marshal, _ := json.Marshal(append([]plugin.Type{p.Type}, p.Requires...))
	...
	for _, p := range plugins {
		...
		if src, ok := instance.(api.GrpcService); ok {
			grpcServices = append(grpcServices, src)
		}
		if src, ok := instance.(api.TtrpcService); ok {
...
func logGRPCJson(method string, request, reply interface{}, err error, path string) {
...
func LoadPlugins(ctx context.Context, config *srvconfig.Config) ([]*plugin.Registration, error) {
	...
	plugin.Register(&plugin.Registration{
		...
		InitFn: func(ic *plugin.InitContext) (interface{}, error) {
			ic.Meta.Exports["root"] = ic.Root // /var/lib/containerd/io.containerd.content.v1.content
...
func (pc *proxyClients) getClient(address string) (*grpc.ClientConn, error) {
	...
	gopts := []grpc.DialOption{
		...
		grpc.WithContextDialer(dialer2.ContextDialer),
	...
	conn, err := grpc.Dial(dialer2.DialAddress(address), gopts...)
	if err != nil {
