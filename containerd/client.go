...
import (
	...
	"demo/pkg/leases"
	leasesproxy "demo/pkg/leases/proxy"
	...
	"demo/containers"
	"demo/content"
	contentproxy "demo/content/proxy"
	...
	sandboxsapi "demo/pkg/api/services/sandbox/v1"
	snapshotsapi "demo/pkg/api/services/snapshots/v1"
	versionservice "demo/pkg/api/services/version/v1"
	apitypes "demo/pkg/api/types"
	"demo/pkg/defaults"
	"demo/pkg/dialer"
	"demo/pkg/events"
	"demo/remotes"
...
func New(address string, opts ...ClientOpt) (*Client, error) {
	...
	if copts.defaultPlatform != nil {
		...
		c.platform = over_platforms.Default()
	}
	...
	if copts.services == nil && c.conn == nil {
		return nil, fmt.Errorf("no grpc connection or services is available: %w", over_errdefs.ErrUnavailable)
	}
...
func NewWithConn(conn *grpc.ClientConn, opts ...ClientOpt) (*Client, error) {
	...
	c := &Client{
		defaultns: copts.defaultns,
		conn:      conn,
		runtime:   fmt.Sprintf("%s.%s", over_plugin.RuntimePlugin, runtime.GOOS),
	}
...
func (c *Client) Reconnect() error {
	if c.connector == nil {
		return fmt.Errorf("unable to reconnect to containerd, no connector available: %w", over_errdefs.ErrUnavailable)
	}
	c.connMu.Lock()
...
func (c *Client) IsServing(ctx context.Context) (bool, error) {
	...
	if c.conn == nil {
		c.connMu.Unlock()
		return false, fmt.Errorf("no grpc connection available: %w", over_errdefs.ErrUnavailable)
	}
	c.connMu.Unlock()
...
type RemoteContext struct {
	...
	HandlerWrapper func(over_images.Handler) over_images.Handler
...
func (c *Client) Fetch(ctx context.Context, ref string, opts ...RemoteOpt) (over_images.Image, error) {
	...
	if fetchCtx.Unpack {
		return over_images.Image{}, fmt.Errorf("unpack on fetch not supported, try pull: %w", over_errdefs.ErrNotImplemented)
	}

	if fetchCtx.PlatformMatcher == nil {
		if len(fetchCtx.Platforms) == 0 {
			fetchCtx.PlatformMatcher = over_platforms.All
		} else {
			var ps []ocispec.Platform
			for _, s := range fetchCtx.Platforms {
				p, err := over_platforms.Parse(s)
				if err != nil {
					return over_images.Image{}, fmt.Errorf("invalid platform %s: %w", s, err)
				}
				ps = append(ps, p)
			}

			fetchCtx.PlatformMatcher = over_platforms.Any(ps...)
		}
	}
...
func (c *Client) Push(ctx context.Context, ref string, desc ocispec.Descriptor, opts ...RemoteOpt) error {
	...
	if pushCtx.PlatformMatcher == nil {
		if len(pushCtx.Platforms) > 0 {
			...
			for _, platform := range pushCtx.Platforms {
				p, err := over_platforms.Parse(platform)
				if err != nil {
					return fmt.Errorf("invalid platform %s: %w", platform, err)
				}
				ps = append(ps, p)
			}
			pushCtx.PlatformMatcher = over_platforms.Any(ps...)
		} else {
			pushCtx.PlatformMatcher = over_platforms.All
		}
	}
	...
	if len(pushCtx.BaseHandlers) > 0 {
		wrapper = func(h over_images.Handler) over_images.Handler {
			h = over_images.Handlers(append(pushCtx.BaseHandlers, h)...)
...
func (c *Client) GetLabel(ctx context.Context, label string) (string, error) {
	ns, err := namespaces2.NamespaceRequired(ctx)
	if err != nil {
		if c.defaultns == "" {
			return "", err
		}
...
func (c *Client) NamespaceService() namespaces2.Store {
...
func (c *Client) ImageService() over_images.Store {
	if c.imageStore != nil {
		return c.imageStore
	}
	c.connMu.Lock()
...
func (c *Client) Version(ctx context.Context) (Version, error) {
	...
	if c.conn == nil {
		c.connMu.Unlock()
		return Version{}, fmt.Errorf("no grpc connection available: %w", over_errdefs.ErrUnavailable)
	}
	c.connMu.Unlock()
...
func (c *Client) Server(ctx context.Context) (ServerInfo, error) {
	...
	if c.conn == nil {
		c.connMu.Unlock()
		return ServerInfo{}, fmt.Errorf("no grpc connection available: %w", over_errdefs.ErrUnavailable)
	}
	c.connMu.Unlock()
...
func (c *Client) getSnapshotter(ctx context.Context, name string) (snapshots.Snapshotter, error) {
	...
	if s == nil {
		return nil, fmt.Errorf("snapshotter %s was not found: %w", name, over_errdefs.ErrNotFound)
	}
...
func (c *Client) GetSnapshotterSupportedPlatforms(ctx context.Context, snapshotterName string) (over_platforms.MatchComparer, error) {
	filters := []string{fmt.Sprintf("type==%s, id==%s", over_plugin.SnapshotPlugin, snapshotterName)}
	...
	return over_platforms.Any(snPlatforms...), nil
}
