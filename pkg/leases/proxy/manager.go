...
func (pm *proxyManager) Create(ctx context.Context, opts ...leases.Opt) (leases.Lease, error) {
	...
	if err != nil {
		return leases.Lease{}, over_errdefs.FromGRPC(err)
	}

	return leases.Lease{
		ID:        resp.Lease.ID,
		CreatedAt: over_protobuf.FromTimestamp(resp.Lease.CreatedAt),
...
func (pm *proxyManager) List(ctx context.Context, filters ...string) ([]leases.Lease, error) {
	...
	for i := range resp.Leases {
		l[i] = leases.Lease{
			ID:        resp.Leases[i].ID,
			CreatedAt: over_protobuf.FromTimestamp(resp.Leases[i].CreatedAt),
