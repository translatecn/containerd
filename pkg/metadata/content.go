...
func (cs *contentStore) Info(ctx context.Context, dgst digest.Digest) (content.Info, error) {
	...
	if err := view(ctx, cs.db, func(tx *bolt.Tx) error {
		...
		if bkt == nil {
			return fmt.Errorf("content digest %v: %w", dgst, over_errdefs.ErrNotFound)
		}
...
func (cs *contentStore) Update(ctx context.Context, info content.Info, fieldpaths ...string) (content.Info, error) {
	...
	if err := update(ctx, cs.db, func(tx *bolt.Tx) error {
		...
		if bkt == nil {
			return fmt.Errorf("content digest %v: %w", info.Digest, over_errdefs.ErrNotFound)
		}
		...
		if len(fieldpaths) > 0 {
			for _, path := range fieldpaths {
				...
				default:
					return fmt.Errorf("cannot update %q field on content info %q: %w", path, info.Digest, over_errdefs.ErrInvalidArgument)
				}
			}
		} else {
			// Set mutable fields
...
func (cs *contentStore) Status(ctx context.Context, ref string) (content.Status, error) {
	...
	if err := view(ctx, cs.db, func(tx *bolt.Tx) error {
		...
		if bref == "" {
			return fmt.Errorf("reference %v: %w", ref, over_errdefs.ErrNotFound)
		}
...
func (cs *contentStore) Writer(ctx context.Context, opts ...content.WriterOpt) (content.Writer, error) {
	...
	if exists {
		return nil, fmt.Errorf("content %v: %w", wOpts.Desc.Digest, over_errdefs.ErrAlreadyExists)
	}
...
func (nw *namespacedWriter) commit(ctx context.Context, tx *bolt.Tx, size int64, expected digest.Digest, opts ...content.Opt) (digest.Digest, error) {
	...
	if nw.w == nil {
		if size != 0 && size != nw.desc.Size {
			return "", fmt.Errorf("%q failed size validation: %v != %v: %w", nw.ref, nw.desc.Size, size, over_errdefs.ErrFailedPrecondition)
		}
		if expected != "" && expected != nw.desc.Digest {
			return "", fmt.Errorf("%q unexpected digest: %w", nw.ref, over_errdefs.ErrFailedPrecondition)
		}
		size = nw.desc.Size
		...
		if size != 0 && size != status.Offset {
			nw.w.Close()
			return "", fmt.Errorf("%q failed size validation: %v != %v: %w", nw.ref, status.Offset, size, over_errdefs.ErrFailedPrecondition)
		}
		size = status.Offset

		if err := nw.w.Commit(ctx, size, expected); err != nil && !over_errdefs.IsAlreadyExists(err) {
			return "", err
		}
		actual = nw.w.Digest()
	}
	...
	if err != nil {
		if err == bolt.ErrBucketExists {
			return actual, fmt.Errorf("content %v: %w", actual, over_errdefs.ErrAlreadyExists)
		}
		return "", err
	}
