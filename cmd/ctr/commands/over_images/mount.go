...
`,
	Flags: append(append(commands.RegistryFlags, append(commands.SnapshotterFlags, commands.LabelFlag)...),
		...
		cli.StringFlag{
			...
			Value: over_platforms.DefaultString(),
		},
	),
	Action: func(context *cli.Context) (retErr error) {
		...
		i := containerd.NewImageWithPlatform(client, img, over_platforms.Only(p))
		...
		if err := mount.All(mounts, target); err != nil {
			if err := s.Remove(ctx, target); err != nil && !over_errdefs.IsNotFound(err) {
