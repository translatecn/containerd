...
func (m *Mount) mount(target string) (err error) {
	...
	if flags&ptypes != 0 {
		...
		if err := my_mount.Mount("", target, "", uintptr(flags&pflags), ""); err != nil {
			return err
		}
	}
	...
	if oflags&broflags == broflags {
		...
		return my_mount.Mount("", target, "", uintptr(oflags|unprivFlags|unix.MS_REMOUNT), "")
	}
	return nil
}
...
// https://github.com/containerd/pull/3765#discussion_r342083514
...
func mountAt(chdir string, source, target, fstype string, flags uintptr, data string) error {
	...
	go func() {
		...
		ch <- my_mount.Mount(source, target, fstype, flags, data)
	}()
	return <-ch
}
