...
func (c *Container) All() (o []process2.Process) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Container) ExecdProcesses() (o []process2.Process) {
	c.mu.Lock()
	defer c.mu.Unlock()
	for _, p := range c.processes {
		o = append(o, p)
...
func (c *Container) Process(id string) (process2.Process, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if id == "" {
		if c.process == nil {
...
func (c *Container) ProcessAdd(process process2.Process) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Container) Delete(ctx context.Context, r *task.DeleteRequest) (process2.Process, error) {
...
func (c *Container) Exec(ctx context.Context, r *task.ExecProcessRequest) (process2.Process, error) {
	process, err := c.process.(*process2.Init).Exec(ctx, c.Bundle, &process2.ExecConfig{
		ID:       r.ExecID,
		Terminal: r.Terminal,
		Stdin:    r.Stdin,
		Stdout:   r.Stdout,
...
func (c *Container) Checkpoint(ctx context.Context, r *task.CheckpointTaskRequest) error {
	...
	return p.(*process2.Init).Checkpoint(ctx, &process2.CheckpointConfig{
		Path:                     r.Path,
		Exit:                     opts.Exit,
...
func (c *Container) Update(ctx context.Context, r *task.UpdateTaskRequest) error {
	...
	return p.(*process2.Init).Update(ctx, r.Resources)
}
...
func newInit(ctx context.Context, path, workDir, namespace string, platform stdio2.Platform,
	r *process2.CreateConfig, options *runc.Options, rootfs string) (*process2.Init, error) {
	runtime := process2.NewRunc(options.Root, path, namespace, options.BinaryName, options.SystemdCgroup)
...
func (c *Container) Start(ctx context.Context, r *task.StartRequest) (process2.Process, error) {
	p, err := c.Process(r.ExecID)
	if err != nil {
		return nil, err
	}
...
func (c *Container) Pause(ctx context.Context) error {
	return c.process.(*process2.Init).Pause(ctx)
}

// Resume the container
func (c *Container) Resume(ctx context.Context) error {
	return c.process.(*process2.Init).Resume(ctx)
}
