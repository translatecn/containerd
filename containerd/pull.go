...
func (c *Client) Pull(ctx context.Context, ref string, opts ...RemoteOpt) (_ Image, retErr error) {
	ctx, span := over_tracing.StartSpan(ctx, over_tracing.Name(pullSpanPrefix, "Pull"))
	defer span.End()
	...
	if pullCtx.PlatformMatcher == nil {
		if len(pullCtx.Platforms) > 1 {
			...
			p, err := over_platforms.Parse(pullCtx.Platforms[0])
			if err != nil {
				return nil, fmt.Errorf("invalid platform %s: %w", pullCtx.Platforms[0], err)
			}

			pullCtx.PlatformMatcher = over_platforms.Only(p)
		}
	}

	span.SetAttributes(
		over_tracing.Attribute("image.ref", ref),
		over_tracing.Attribute("unpack", pullCtx.Unpack),
		over_tracing.Attribute("max.concurrent.downloads", pullCtx.MaxConcurrentDownloads),
		over_tracing.Attribute("platforms.count", len(pullCtx.Platforms)),
	)
	...
	if pullCtx.Unpack {
		...
		span.SetAttributes(over_tracing.Attribute("snapshotter.name", snapshotterName))
		...
		var platformMatcher over_platforms.Matcher
		...
		pullCtx.HandlerWrapper = func(h over_images.Handler) over_images.Handler {
			if wrapper == nil {
	...
	if unpacker != nil {
		_, unpackSpan := over_tracing.StartSpan(ctx, over_tracing.Name(pullSpanPrefix, "UnpackWait"))
	...
	span.SetAttributes(over_tracing.Attribute("image.ref", i.Name()))
...
func (c *Client) fetch(ctx context.Context, rCtx *RemoteContext, ref string, limit int) (over_images.Image, error) {
	ctx, span := over_tracing.StartSpan(ctx, over_tracing.Name(pullSpanPrefix, "fetch"))
	defer span.End()
	store := c.ContentStore()
	name, desc, err := rCtx.Resolver.Resolve(ctx, ref)
	if err != nil {
		return over_images.Image{}, fmt.Errorf("failed to resolve reference %q: %w", ref, err)
	}

	fetcher, err := rCtx.Resolver.Fetcher(ctx, name)
	if err != nil {
		return over_images.Image{}, fmt.Errorf("failed to get fetcher for %q: %w", name, err)
	}
	...
	if desc.MediaType == over_images.MediaTypeDockerSchema1Manifest && rCtx.ConvertSchema1 {
		schema1Converter := schema1.NewConverter(store, fetcher)

		handler = over_images.Handlers(append(rCtx.BaseHandlers, schema1Converter)...)
		...
		childrenHandler = over_images.SetChildrenMappedLabels(store, childrenHandler, rCtx.ChildLabelMap)
		if rCtx.AllMetadata {
			...
			childrenHandler = over_images.FilterPlatforms(childrenHandler, rCtx.PlatformMatcher)
		}
		// Sort and limit manifests if a finite number is needed
		if limit > 0 {
			childrenHandler = over_images.LimitManifests(childrenHandler, rCtx.PlatformMatcher, limit)
		}

		// set isConvertible to true if there is application/octet-stream media type
		convertibleHandler := over_images.HandlerFunc(
		...
		handler = over_images.Handlers(handlers...)
	...
	if originalSchema1Digest != "" {
		...
		rCtx.Labels[over_images.ConvertedDockerSchema1LabelKey] = originalSchema1Digest
	}
...
func (c *Client) createNewImage(ctx context.Context, img over_images.Image) (over_images.Image, error) {
	ctx, span := over_tracing.StartSpan(ctx, over_tracing.Name(pullSpanPrefix, "pull.createNewImage"))
	defer span.End()
	is := c.ImageService()
	for {
