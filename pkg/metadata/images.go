...
func NewImageStore(db *DB) over_images.Store {
	return &imageStore{db: db}
}

func (s *imageStore) Get(ctx context.Context, name string) (over_images.Image, error) {
	var image over_images.Image
	...
	if err := view(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if bkt == nil {
			return fmt.Errorf("image %q: %w", name, over_errdefs.ErrNotFound)
		}
...
func (s *imageStore) List(ctx context.Context, fs ...string) ([]over_images.Image, error) {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("%s: %w", err.Error(), over_errdefs.ErrInvalidArgument)
	}

	var m []over_images.Image
	if err := view(ctx, s.db, func(tx *bolt.Tx) error {
		bkt := getImagesBucket(tx, namespace)
		if bkt == nil {
...
func (s *imageStore) Create(ctx context.Context, image over_images.Image) (over_images.Image, error) {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return over_images.Image{}, err
	}
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if err != nil {
			...
			return fmt.Errorf("image %q: %w", image.Name, over_errdefs.ErrAlreadyExists)
		}
...
func (s *imageStore) Update(ctx context.Context, image over_images.Image, fieldpaths ...string) (over_images.Image, error) {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return over_images.Image{}, err
	}
	...
	if image.Name == "" {
		return over_images.Image{}, fmt.Errorf("image name is required for update: %w", over_errdefs.ErrInvalidArgument)
	}

	var updated over_images.Image
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if ibkt == nil {
			return fmt.Errorf("image %q: %w", image.Name, over_errdefs.ErrNotFound)
		}
		...
		if len(fieldpaths) > 0 {
			for _, path := range fieldpaths {
				...
				default:
					return fmt.Errorf("cannot update %q field on image %q: %w", path, image.Name, over_errdefs.ErrInvalidArgument)
				}
			}
		} else {
			updated = image
...
func (s *imageStore) Delete(ctx context.Context, name string, opts ...over_images.DeleteOpt) error {
	namespace, err := namespaces.NamespaceRequired(ctx)
	if err != nil {
		return err
	}
	...
	return update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if err = bkt.DeleteBucket([]byte(name)); err != nil {
			if err == bolt.ErrBucketNotFound {
				err = fmt.Errorf("image %q: %w", name, over_errdefs.ErrNotFound)
			}
			return err
		}
...
func validateImage(image *over_images.Image) error {
	if image.Name == "" {
		return fmt.Errorf("image name must not be empty: %w", over_errdefs.ErrInvalidArgument)
	}
...
func validateTarget(target *ocispec.Descriptor) error {
	...
	if err := target.Digest.Validate(); err != nil {
		return fmt.Errorf("Target.Digest %q invalid: %v: %w", target.Digest, err, over_errdefs.ErrInvalidArgument)
	}

	if target.Size <= 0 {
		return fmt.Errorf("Target.Size must be greater than zero: %w", over_errdefs.ErrInvalidArgument)
	}

	if target.MediaType == "" {
		return fmt.Errorf("Target.MediaType must be set: %w", over_errdefs.ErrInvalidArgument)
	}

	return nil
}

func readImage(image *over_images.Image, bkt *bolt.Bucket) error {
...
func writeImage(bkt *bolt.Bucket, image *over_images.Image) error {
