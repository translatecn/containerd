...
import (
	...
	defaults2 "demo/over/defaults"
	"demo/over/leases"
	leasesproxy "demo/over/leases/proxy"
	...
	contentapi "demo/over/api/services/content/v1"
	...
	leasesapi "demo/over/api/services/leases/v1"
	namespacesapi "demo/over/api/services/namespaces/v1"
	sandboxsapi "demo/over/api/services/sandbox/v1"
	snapshotsapi "demo/over/api/services/snapshots/v1"
	versionservice "demo/over/api/services/version/v1"
	apitypes "demo/over/api/types"
	"demo/over/containers"
	"demo/over/content"
	contentproxy "demo/over/content/proxy"
	"demo/over/errdefs"
	"demo/over/events"
	"demo/over/images"
	"demo/over/platforms"
...
func New(address string, opts ...ClientOpt) (*Client, error) {
	...
	if copts.defaultRuntime != "" {
		...
		c.runtime = defaults2.DefaultRuntime
	}
	...
	if copts.defaultRuntime == "" && c.defaultns != "" {
		if label, err := c.GetLabel(context.Background(), defaults2.DefaultRuntimeNSLabel); err != nil {
			return nil, err
		} else if label != "" {
			c.runtime = label
		}
...
// NewContainer will create a new container with the provided randomId.
// The randomId must be unique within the namespace.
...
func (c *Client) resolveSnapshotterName(ctx context.Context, name string) (string, error) {
	if name == "" {
		label, err := c.GetLabel(ctx, defaults2.DefaultSnapshotterNSLabel)
		if err != nil {
			return "", err
		}
