	...
	Action: func(context *cli.Context) error {
		...
		if manifest := context.String("manifest"); manifest != "" {
			...
			if pss := context.StringSlice("platform"); len(pss) == 1 {
				p, err := over_platforms.Parse(pss[0])
				if err != nil {
					return fmt.Errorf("invalid platform %q: %w", pss[0], err)
				}

				cs := client.ContentStore()
				if manifests, err := over_images.Children(ctx, cs, desc); err == nil && len(manifests) > 0 {
					matcher := over_platforms.NewMatcher(p)
					for _, manifest := range manifests {
						if manifest.Platform != nil && matcher.Match(*manifest.Platform) {
							if _, err := over_images.Children(ctx, cs, manifest); err != nil {
		...
		eg.Go(func() error {
			...
			jobHandler := over_images.HandlerFunc(func(ctx gocontext.Context, desc ocispec.Descriptor) ([]ocispec.Descriptor, error) {
				if !context.Bool("allow-non-distributable-blobs") && over_images.IsNonDistributable(desc.MediaType) {
					return nil, nil
				}
